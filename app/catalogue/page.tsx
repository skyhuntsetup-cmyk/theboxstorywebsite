...
export default function CataloguePage() {
  ...
  // Restore a previously-started visit (same device/session) so a page
  // refresh doesn't lose the customer's name/WhatsApp entry. sessionStorage
  // is only reachable client-side, so this can only run post-mount.
  useEffect(() => {
  ...
  const handleStartBrowsing = (e: React.FormEvent) => {
    ...
    sessionStorage.setItem(LEAD_STORAGE_KEY, JSON.stringify({ name: name.trim(), whatsapp: whatsapp.trim() }));
    setStep("browse");
  };
  ...
  const handleCheckout = async () => {
    ...
    try {
      // 1. Generate a branded order-summary PDF and download it.
      const doc = new jsPDF();
      const brandName = "The Box Story";
      let y = 20;
      ...
      doc.text("Gift Catalogue Order Summary", 14, y);
      y += 10;
      ...
      doc.setFont("helvetica", "bold");
      doc.setFontSize(12);
      doc.text(`Total: Rs ${subtotal}`, 150, y);

      const fileName = `TheBoxStory-Order-${Date.now()}.pdf`;
      doc.save(fileName);

      // 2. Save the lead + cart snapshot so the business has a record even
      // if the customer doesn't complete the WhatsApp step.
      fetch("/api/catalogue", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ name, whatsapp, cart_items: cart, subtotal }),
      }).catch(() => { /* non-blocking */ });

      // 3. Open WhatsApp with a pre-filled order summary. WhatsApp's web
      // deep link can't auto-attach a file, so the message asks the
      // customer to attach the PDF that just downloaded.
      const businessNumber = getContent("catalogue.whatsappNumber", "917838783488");
      const lines = cart.map((i) => `• ${i.name} x${i.quantity} — ₹${i.price * i.quantity}`).join("\n");
      const message =
        `Hi The Box Story! I'd like to order:\n\n${lines}\n\nTotal: ₹${subtotal}\n\n` +
        `My name: ${name}\nMy WhatsApp: ${whatsapp}\n\n` +
        `(I've just downloaded my order PDF — attaching it here.)`;
      ...
      <div className="min-h-[85vh] flex items-center justify-center px-6 py-16">
        <div className="w-full max-w-md bg-white border border-slate-200 rounded-[32px] p-8 shadow-sm space-y-6 text-center">
          ...
          <div className="space-y-2">
            <h1 className="font-heading text-2xl font-black text-teal-deep">Browse Our Gift Catalogue</h1>
            <p className="text-xs text-slate-500">Tell us who&apos;s shopping, then explore and build your order — we&apos;ll pick it up over WhatsApp.</p>
          </div>
          <form onSubmit={handleStartBrowsing} className="space-y-4 text-left">
            ...
            <button type="submit" className="w-full py-3.5 bg-teal-deep hover:bg-teal-deep/90 text-white rounded-xl font-bold text-sm transition-colors flex items-center justify-center space-x-2">
    ...
    <div className="min-h-screen bg-background text-slate-800 py-10 px-6 pb-32">
      <div className="max-w-6xl mx-auto space-y-8">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            <h1 className="font-heading text-2xl sm:text-3xl font-black text-teal-deep">Hi {name.split(" ")[0]}, welcome!</h1>
            <p className="text-xs text-slate-500">Add anything you like to your list, then share it with us on WhatsApp.</p>
          </div>
          <button
        ...
        <div className="fixed inset-0 z-50 flex justify-end">
          ...
          <div className="relative w-full max-w-md bg-white h-full shadow-2xl flex flex-col">
            <div className="flex items-center justify-between p-6 border-b border-teal-deep/5">
              <h3 className="font-heading text-lg font-bold text-teal-deep">Your List</h3>
            ...
            <div className="p-6 border-t border-teal-deep/5 space-y-3">
              ...
              <button
                ...
                className="w-full py-3.5 bg-[#25D366] hover:bg-[#1fb855] text-white rounded-xl font-bold text-sm transition-colors disabled:opacity-50 flex items-center justify-center space-x-2"
              >
                {isCheckingOut ? <Loader className="w-4 h-4 animate-spin" /> : (
                  <>
                    <MessageCircle className="w-4 h-4" />
                    <span>Get PDF & Share on WhatsApp</span>
                  </>
                )}
              </button>
