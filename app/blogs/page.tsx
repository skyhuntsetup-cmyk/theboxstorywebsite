    ...
    <div className="min-h-screen bg-background text-slate-800 py-10 px-6">
      <div className="max-w-5xl mx-auto space-y-10">
        ...
        <motion.section
          ...
          className="relative rounded-[40px] overflow-hidden bg-white border border-slate-200 p-8 md:p-16 shadow-sm text-left"
        >
          ...
          <div className="max-w-2xl space-y-4">
            <span className="text-[12px] tracking-widest font-black uppercase text-saffron bg-saffron/10 px-3 py-1.5 rounded-full inline-block border border-saffron/20">
          ...
          <div className="flex flex-wrap gap-2">
            {categories.map((cat) => (
              <button
                ...
                className={`text-[13px] font-bold px-4 py-2.5 rounded-full border transition-all ${
        ...
        <motion.div layout className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          ...
          {isLoading ? (
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="col-span-full py-14 text-center space-y-3">
              <BookOpen className="w-10 h-10 text-slate-300 mx-auto" />
              <p className="text-sm font-semibold text-slate-550">No articles match your search query.</p>
            </motion.div>
          ) : (
            filteredPosts.map((post) => (
              <motion.div key={post.slug} layout initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -15 }} transition={{ duration: 0.3 }}>
              <Link
              ...
              >
                ...
                <div className="relative h-48 bg-slate-50 border-b border-slate-100 flex items-center justify-center p-4">
                  ...
                  <div className="absolute top-4 left-4 bg-white/90 backdrop-blur-sm border border-slate-200/60 text-[10px] font-black uppercase tracking-widest text-slate-600 px-2.5 py-1 rounded-full shadow-sm">
                    {post.category}
                  </div>
                </div>
                ...
                <div className="p-6 flex-1 flex flex-col justify-between text-left space-y-4">
                  <div className="space-y-2">
                    <div className="flex items-center space-x-3 text-[12px] font-mono text-slate-400">
                  ...
                  <div className="flex items-center justify-between pt-3 border-t border-slate-100 mt-auto text-[13px] font-bold text-teal-deep group-hover:text-saffron transition-colors">
        ...
        <motion.section {...revealProps} className="bg-white border border-slate-200/60 rounded-[32px] p-8 text-center space-y-4 shadow-sm">
